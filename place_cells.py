...
class PlaceCells(object):
    def __init__(self, options, us=None):
        ...
        self.us = torch.tensor(np.vstack([usx, usy]).T)  # (512, 2)
    ...
    def grid_pc(self, pc_outputs, res=32):
        ...
        for i in range(len(pc_outputs)):
            # gridval = scipy.interpolate.griddata(self.us.cpu(), pc_outputs[i])
            gridval = scipy.interpolate.griddata(self.us.cpu(), pc_outputs[i], grid, method='cubic')
